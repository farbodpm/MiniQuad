...
function install_esp_idf() {
    mkdir -p ${IDF_TOOLS_PATH}/esp-idf/
    NORMALIZED_VERSION=$(echo ${ESP_IDF_VERSION} | sed -e 's!/!-!g')
    export IDF_PATH="${IDF_TOOLS_PATH}/esp-idf/${NORMALIZED_VERSION}"
    git clone --branch ${ESP_IDF_VERSION} --depth 1 --shallow-submodules \
        --recursive https://github.com/espressif/esp-idf.git \
        "${IDF_PATH}"
    cd ${IDF_PATH}
    patch -f components/newlib/platform_include/sys/dirent.h /root/dirnet.patch
    ${IDF_PATH}/install.sh "${BUILD_TARGET}"
    cd ${IDF_PATH}/components
    git submodule add --depth 1 https://github.com/joltwallet/esp_littlefs.git 
